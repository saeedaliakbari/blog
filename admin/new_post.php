...
if ( isset($_POST['add_post']) ) {
    if (trim($_POST['title']) != "" && trim($_POST['auther']) != "" && trim($_POST['category_id']) != "" && trim($_POST['body']) != "" && trim($_FILES['image']['name']) != "" ) {
        ...
        $post_insert = $db->prepare("INSERT INTO posts (title, auther, category_id, body, image) VALUES (:title , :auther , :category_id, :body, :image)");
        $post_insert->execute(['title' => $title, 'auther' => $auther, 'category_id' => $category_id, 'body' => $body, 'image' => $name_image]);
