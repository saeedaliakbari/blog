...
if (isset($_POST['edit_post'])) {
    ...
    if (trim($_POST['title']) != "" && trim($_POST['auther']) != "" && trim($_POST['category_id']) != "" && trim($_POST['body']) != "" ) {
        ...
        if( trim($_FILES['image']['name']) != "" ){
          ...
          $post_update = $db->prepare("UPDATE posts SET title =:title, auther=:auther, category_id=:category_id, body=:body, image=:image WHERE id=:id");
          $post_update->execute(['title' => $title, 'auther' => $auther, 'category_id' => $category_id, 'body' => $body, 'image' => $name_image, 'id' => $post_id]);

        }else{

          $post_update = $db->prepare("UPDATE posts SET title =:title, auther=:auther, category_id=:category_id, body=:body WHERE id=:id");
          $post_update->execute(['title' => $title, 'auther' => $auther, 'category_id' => $category_id, 'body' => $body, 'id' => $post_id]);
    ...
    <div class="row">
        ...
        <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
            ...
            <form method="post" class="mb-5" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label for="auther">نویسنده : </label>
                    <input type="text" class="form-control" value="<?php echo $post['auther'] ?>" name="auther" id="auther">
